      ...
      this.warehouseService.getWarehouseByTracking(tracking).then(result => {
        ...
        if (scannedWarehouse == undefined) {
          ...
          // }
            ...
            this.warehouseService.getDeliveryWarehousesByCustomer(scannedWarehouse.idcustomers).then(result => {
              console.log(result);
              deliveryWarehousesByCustomer = result;
              this.customerService.getCustomerByID(scannedWarehouse.idcustomers).then(result => {
                console.log(result);
                scannedWarehouse.customer = result[0].firstName + " " + result[0].lastName;
                this.checkScanLogDuplicate(scannedWarehouse.idwarehouse).then(result => {
                  if (result == true) {
